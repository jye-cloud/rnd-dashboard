(function () {
  ...
  var fs = window.firestoreService;
  let hrData = fs ? fs.getHrData() : [];
  let editingId = null;
  ...
  // 데이터 로드: Firestore 사용 시 실시간 구독으로 갱신되며, 여기서는 초기/폴백용
  function loadData() {
    try {
      return fs ? fs.getHrData() : (function () {
  ...
  // 데이터 저장: Firestore 또는 LocalStorage에 즉시 반영
  function saveData() {
    try {
  ...
  // Firestore 실시간 구독: 원격 변경 시 테이블·통계 자동 갱신
  if (window.firestoreService && window.firestoreService.subscribeHr) {
    var uiStateLoaded = false;
    window.firestoreService.subscribeHr(function (data) {
      hrData = Array.isArray(data) ? data : [];
      applyFilters();
      renderTable();
      updateStatusCardsUI();
      updateStats();
